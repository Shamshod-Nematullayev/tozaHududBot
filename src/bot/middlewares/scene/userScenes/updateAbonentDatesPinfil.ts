...
import { Abonent as IAbonent, getAbonentById } from '@services/billing/getAbonentById.js';
...
export const updateAbonentDatesByPinfl = new Scenes.WizardScene<Ctx>(
  ...
  async (ctx) => {
    try {
      ...
      if (!ctx.message || !('text' in ctx.message))
        return ctx.reply("kutilgan ma'lumot kiritilmadi", keyboards.cancelBtn.resize());
      ...
      const abonentOnBilling = await getAbonentById(createTozaMakonApi(inspektor.companyId), abonent.id);
      if (!abonentOnBilling) {
        return ctx.reply(
          "Siz kiritgan hisob raqami bo'yicha abonent ma'lumoti topilmadi. Tekshirib qaytadan kiriting",
          keyboards.cancelBtn.resize()
      ...
      if (abonent.shaxsi_tasdiqlandi && abonent.shaxsi_tasdiqlandi.confirm === true) {
        if (
          ...
          ctx.scene.leave();
          return ctx.reply(
            `Ushbu abonent ${abonent.shaxsi_tasdiqlandi.inspector_name} tomonidan allaqachon shaxsi tasdiqlangan!`
          );
        }
        ctx.reply(
          `Ushbu abonent ${abonent.shaxsi_tasdiqlandi.inspector_name} tomonidan allaqachon shaxsi tasdiqlangan! Baribir o'zgartirmoqchimisiz?`,
  ...
  async (ctx) => {
    ...
    if (!isTextMessage(ctx)) return ctx.reply("Kutilgan ma'lumot kiritilmadi", keyboards.cancelBtn.resize());
    ...
    if (!customDates.success) {
      if (customDates.message == "Hozirda baza ishlamayapti, keyinroq boshqatdan urinib ko'ring") {
    ...
    const buffer = customDates.isFromTozaMakon ? customDates.photo : Buffer.from(customDates.photo, 'base64');

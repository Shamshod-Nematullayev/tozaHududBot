...
const accountNumberRegex = [/^\d{12}$/, "uzunligi 12 ta raqamdan iborat matn bo'lishi kerak"] as const;
...
export const createArizaBodySchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if ((data.document_type === 'odam_soni' || data.document_type === 'death') && !data.current_prescribed_cnt) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['current_prescribed_cnt'],
        message: "Odamlar soni, O'lim guvohnomasi arizalariga odamlar soni kiritish majburiy!",
      });
    }
  });
...
export const changeArizaActBodySchema = z.object({
  ...
  photos: z.preprocess((arg) => (typeof arg === 'string' ? JSON.parse(arg) : arg), z.array(z.string()).optional()),
  actNumber: z.string(),
});
...
export const createMonayTransferArizaBodySchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.creditorActs.reduce((total, act) => total + act.amount, 0) !== data.debitorAct.amount) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['creditorActs'],
    ...
    if (data.creditorActs.find((a) => a.accountNumber === data.debitorAct.accountNumber)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['creditorActs'],

...
export function checkPackExist(pack, company) {
  const date = new Date();
  const month = date.getMonth() + 1;
  const year = date.getFullYear();
  const packet = company.akt_pachka_ids[pack];
  if (packet.month == month && packet.year == year) {
    return true;
  }
  return false;
}
export async function createAktPack(companyId) {
  try {
